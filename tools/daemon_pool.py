...
class DaemonThreadPoolExecutor(ThreadPoolExecutor):
    ...
    def _adjust_thread_count(self) -> None:
        ...
        # _threads_queues registration. 3.14 replaced _initializer/_initargs with
        # _create_worker_context(); support both so one class runs on 3.11-3.14.
        ...
        if num_threads < self._max_workers:
            ...
            # right profile (#54937).
            _create_ctx = getattr(self, "_create_worker_context", None)
            if callable(_create_ctx):
                worker_args = (weakref.ref(self, weakref_cb), _create_ctx(), self._work_queue)
            else:
